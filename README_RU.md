...
Проект включает две схемы для различных задач:

| Схема | Назначение | Подписки |
|-------|------------|----------|
| `AIChat` | Продакшен сборка | AppHud SDK (реальные или sandbox покупки) |
| `AIChat-StoreKitTest` | Локальное тестирование | Файл конфигурации StoreKit (без сети) |

Чтобы переключить схему, нажмите на выборщик схем в панели инструментов Xcode и выберите нужную схему.
